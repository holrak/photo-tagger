...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [`main.py`][main]                   | cyclopts entry point: the `tag()` default command (orchestration) plus the `doctor`, `watch`, and `undo` commands. Resolves the batch, acquires the lock, runs the batch, emits NDJSON, and writes the summary.                                                                                                                      |
...
| [`gui.py`][gui]                     | Optional PySide6 desktop frontend (`photo-tagger gui`): a review-before-write window that generates proposals on a background `QThread` (reusing `create_agent`/`analyze_image_with_ai`, not `run_batch`) and writes on demand, batch saves on a second `QThread`. Imported lazily; excluded from coverage and the static analyzers. |
...
| [`vocabulary.py`][vocabulary]       | The controlled vocabulary (`--vocabulary`): parses a Lightroom keyword export or a plain term list, and snaps generated keywords onto the catalog's own spelling and hierarchy.                                                                                                                                                      |
| [`sessions.py`][sessions]           | Groups a batch into shoots by capture time (`--session-gap`) and derives each shoot's vocabulary from its own output, so every frame of one subject agrees.                                                                                                                                                                          |
| [`undo.py`][undo]                   | The per-run write journal and the restore logic behind `photo-tagger undo`.                                                                                                                                                                                                                                                          |
| [`watch.py`][watch]                 | The polling folder watcher behind `photo-tagger watch`: yields each batch of new photos once they have stopped changing.                                                                                                                                                                                                             |
| [`errors.py`][errors]               | `PhotoTaggerError` (base), `ProviderError`, `DiscoveryError`, `BatchError`, `VocabularyError`, `UndoError`.                                                                                                                                                                                                                          |
...
proposal, and only then does the window call `merge_keywords()` and `write_metadata()` to save. A
batch save runs on its own worker thread too, resolving what to write on the UI thread (a `SaveJob`
per photo) and reusing one ExifTool process for the whole batch. Both workers reach the UI thread
only through Qt signals, so the pipeline and the building blocks stay free of any Qt dependency. The
Qt-free parts of the GUI live in [`gui_state.py`][gui_state] so this logic stays testable without a
display.
...
[sessions]: https://github.com/jbsilva/photo-tagger/blob/main/src/photo_tagger/sessions.py
[undo]: https://github.com/jbsilva/photo-tagger/blob/main/src/photo_tagger/undo.py
[vocabulary]: https://github.com/jbsilva/photo-tagger/blob/main/src/photo_tagger/vocabulary.py
[watch]: https://github.com/jbsilva/photo-tagger/blob/main/src/photo_tagger/watch.py
